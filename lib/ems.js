		...
		if (adapter.config.ems_dallas) {
			...
			if (data != "Invalid") {
				...
				for (const [key, value] of Object.entries(sensors)) {
					...
					let def;
					try {
						def = await ems_get(url1);
						write_state("dallassensor."+key,value,def);
					}
					catch(error) {write_state("dallassensor."+key,value,"");}
				}
			}
		}

		if (adapter.config.ems_analog) {
			let url = emsesp +  "/api/analogsensor";
			data = "";
			try {data = await ems_get(url); }
			catch(error) {data = "Invalid";}
			...
			if (data != "Invalid") {
				...
				for (const [key, value] of Object.entries(analogs)) {
					...
					try {
						def = await ems_get(url1);
						write_state("analogsensor."+key,value,def);
					}
					catch(error) {write_state("analogsensor."+key,value,"");}
				}
			}
		}
...
async function ems_read(version) {
	...
	if (adapter.config.ems_dallas) {
		...
		if (data != "Invalid") {
			let sensors = {};
			try {sensors = JSON.parse(data);}
			catch(error) {sensors = "Invalid";}

			if (sensors != "Invalid") {
				for (const [key, value] of Object.entries(sensors)) {
					write_state("dallassensor."+key,value,"");
				}
			}
		}
	}
